...
pipeHeight = 320
pipeWidth = 52
pipeDistance = difficulty.pipeDistance    #上下管道间的距离
pipeInterval = 180    #两根管道的水平距离
waitDistance = 100    #开始时第一根管道距离屏幕最右侧的距离
heightOffset = random.randint(25,75)     #管道的高度偏移值
# vars
PIPE_NEW = 0
PIPE_PASS = 1
...
def createPipes(layer, gameScene, spriteBird, score):
    ...
    def initPipe():
        for i in range(0, pipeCount):
            pipeDistance = difficulty.pipeDistance
            heightOffset = random.randint(10+10*stage,150-10*stage)
    ...
    def movePipe(dt):
        moveDistance = common.visibleSize["width"]/(2*60)-(stage/10)   # 移动速度和land一致
        for i in range(0, pipeCount):
    ...
    def calScore(dt):
        ...
        for i in range(0, pipeCount):
            if pipeState[i] == PIPE_NEW and pipes[i].position[0]< birdXPosition:
                ...
                g_score = g_score + 1
                game_controller.sendScore(g_score)
                storeScore(g_score)
...
def storeScore(score):
    ...
    output = open('./score/_'+user+'.txt', 'w+')
